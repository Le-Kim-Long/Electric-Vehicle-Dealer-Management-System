...
const VehicleInfoFeature = () => {
  ...
  useEffect(() => {
    if (searchTerm) {
      ...
      // ✅ Không có search → áp dụng filter local
      applyLocalFilters();
    }
  }, [searchTerm]);

  // ✅ Apply filter khi thay đổi brand/version (chỉ khi KHÔNG search)
  useEffect(() => {
    if (!searchTerm) {
      applyLocalFilters();
    }
  ...
  // ✅ Function để filter local
  const applyLocalFilters = () => {
    let filtered = [...vehicles];

    if (filterBrand !== 'all') {
      filtered = filtered.filter(vehicle => 
        vehicle.modelName.toLowerCase() === filterBrand.toLowerCase()
      );
    }

    if (filterVersion !== 'all') {
      filtered = filtered.filter(vehicle => 
        vehicle.variantName.toLowerCase() === filterVersion.toLowerCase()
      );
    }

    console.log('✅ Applied local filters:', filtered.length, 'vehicles');
