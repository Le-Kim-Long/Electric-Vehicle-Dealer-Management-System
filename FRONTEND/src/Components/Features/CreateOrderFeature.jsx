...
const CreateOrderFeature = () => {
  ...
  const calculateDiscount = () => {
    ...
    if (orderData.promotion.loai === 'VNĐ') {
      return orderData.promotion.giaTri;
    } else if (orderData.promotion.loai === '%') {
      return subtotal * (orderData.promotion.giaTri / 100);
    }
    return 0;
  };
  ...
  const submitOrder = () => {
    try {
      ...
      if (!orderData.payment.phuongThuc) {
      ...
      const newOrder = {
        ...
        paymentMethod: orderData.payment.phuongThuc,
...
// SỬA LẠI PaymentStep - thay {{ thành [
        ...
        {['Tiền mặt', 'Chuyển khoản', 'Thẻ tín dụng'].map((method) => (
          <label key={method} className="payment-method-option">
            <input
              ...
              value={method}
              checked={orderData.payment.phuongThuc === method}
              onChange={(e) => setOrderData(prev => ({
                ...prev,
                payment: { ...prev.payment, phuongThuc: e.target.value }
              }))}
            />
            <span>{method}</span>
          </label>
  ...
  return (
    <div className="step-content">
      ...
      <div className="order-summary">
        ...
        <div className="summary-section">
          <h4>Thanh toán</h4>
          <p><strong>Hình thức tài chính:</strong> {orderData.financing.phuongThucThanhToan}</p>
          <p><strong>Phương thức thanh toán:</strong> {orderData.payment.phuongThuc}</p>
