...
public class OrdersServiceImpl implements OrdersService {
    ...
    public List<DealerOrderResponse> getOrdersByCurrentUserDealer(String currentUserEmail) {
        ...
        if ("DealerManager".equals(roleName)) {
            // Manager: Lấy tất cả orders của dealer
            orders = ordersRepository.findAllByDealerId(dealerId);
        } else {
            // DealerStaff: Chỉ lấy orders được tạo bởi user đó
            orders = ordersRepository.findAllByDealerIdAndUserId(dealerId, currentUser.getUserId());
        }
     ...
     */
    @Transactional(readOnly = true)
    public List<DealerOrderResponse> getOrdersByCreatorName(String creatorName, String currentUserEmail) {
        // Validate input
        if (creatorName == null || creatorName.trim().isEmpty()) {
            throw new RuntimeException("Creator name is required");
        }
        ...
        // Kiểm tra role - chỉ cho phép DealerManager
        if (!"DealerManager".equals(roleName)) {
            throw new RuntimeException("Access denied. Only DealerManager can search orders by creator name.");
        }
        ...
        // Lấy orders theo tên người tạo
        List<Orders> orders = ordersRepository.findAllByDealerIdAndCreatorName(dealerId, creatorName.trim());
    ...
    private DealerOrderResponse convertToOrderResponse(Orders order) {
        ...
        if (order.getUserAccount() != null) {
            createdBy = order.getUserAccount().getUsername();
        }
        ...
        OrderInfoResponse orderInfo = OrderInfoResponse.builder()
                ...
                .createdBy(createdBy) // Thêm tên người tạo đơn
                .build();
    ...
    public CreateDraftOrderResponse createDraftOrder(CreateDraftOrderRequest request, String currentUserEmail) {
        ...
        draftOrder.setUserAccount(currentUser); // Lưu thông tin người dùng tạo đơn
    ...
    public UpdateOrderPromotionResponse updateOrderPromotion(Integer orderId, UpdateOrderPromotionRequest request, String currentUserEmail) {
        ...
        if ("VND".equalsIgnoreCase(promotion.getType())) {
            ...
            BigDecimal percentage = promotion.getValue().divide(BigDecimal.valueOf(100), 4, RoundingMode.HALF_UP);
    ...
    private void validateUpdateOrderPaymentMethodRequest(Integer orderId, UpdateOrderPaymentMethodRequest request) {
        ...
        if (!isValidPaymentMethod(paymentMethod)) {
            throw new RuntimeException("Invalid payment method. Supported methods: Trả thẳng");
        }
    }
    ...
    public UpdateOrderStatusResponse updateOrderStatus(Integer orderId, UpdateOrderStatusRequest request, String currentUserEmail) {
        ...
        // Xử lý hoàn trả xe về kho khi đơn hàng bị hủy
        if ("Đã hủy".equals(request.getStatus()) && !"Đã hủy".equals(oldStatus)) {
            // Lấy danh sách order details để hoàn trả xe
            ...
            for (OrderDetails orderDetail : orderDetailsList) {
                ...
                // Tìm DealerCar để hoàn trả số lượng
                DealerCar dealerCar = dealerCarRepository.findByCarIdAndDealerId(carId, dealerId)
                        .orElseThrow(() -> new RuntimeException("Dealer car record not found for carId: " + carId));
        ...
        // Tự động cập nhật completedDate khi status chuyển thành "Đã thanh toán"
        if ("Đã thanh toán".equals(request.getStatus()) && !"Đã thanh toán".equals(oldStatus)) {
            order.setCompletedDate(LocalDateTime.now());
        }
        // Nếu status không phải "Đã thanh toán" thì xóa completedDate
        else if (!"Đã thanh toán".equals(request.getStatus()) && order.getCompletedDate() != null) {
        ...
        return UpdateOrderStatusResponse.builder()
                ...
                .orderDate(savedOrder.getOrderDate())
                .completedDate(savedOrder.getCompletedDate())
    ...
    private void validateUpdateOrderStatusRequest(Integer orderId, UpdateOrderStatusRequest request) {
        ...
        if (!isValidOrderStatus(status)) {
            throw new RuntimeException("Invalid order status. Supported statuses: Chưa xác nhận, Chưa thanh toán, Đã thanh toán, Đã hủy");
        }
    }
    ...
    private boolean isValidOrderStatus(String status) {
        ...
        String[] validStatuses = {"Chưa xác nhận", "Chưa thanh toán","Đã thanh toán", "Đã hủy"};
