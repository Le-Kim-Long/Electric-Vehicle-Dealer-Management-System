...
public class PromotionService {
    ...
    public List<PromotionResponse> getActivePromotionsByDealerManager(String email) {
        // Find user by email
        UserAccount user = userAccountRepository.findByEmail(email)
                .orElseThrow(() -> new RuntimeException("User not found with email: " + email));
        ...
        // Get active promotions for the dealer
        List<Promotion> promotions = promotionRepository.findByDealerIdAndStatus(
                user.getDealer().getDealerId(), "Active");
    ...
    public List<PromotionResponse> searchPromotionsByValue(String email, BigDecimal searchValue) {
        // Find user by email
        UserAccount user = userAccountRepository.findByEmail(email)
                .orElseThrow(() -> new RuntimeException("User not found with email: " + email));
        ...
        // Filter promotions by value
        List<Promotion> filteredPromotions = promotions.stream()
                .filter(promotion -> promotion.getValue() != null &&
                        promotion.getValue().compareTo(searchValue) == 0)
                .collect(Collectors.toList());
